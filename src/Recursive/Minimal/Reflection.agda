...
module Recursive.Minimal.Reflection where
...
open import Recursive.Presupposition using
  ...
  ; natStepCompSub ; natStepCompSubCtx ; natStepCompSubCtx-subTm
  ; natStepArgTyComp ; natStepTyComp
...
minNatStepFits : {gamma : Ctx} {L : RawType} {n rec : RawTerm}
  -> Minimal (hasTy gamma n tyNat)
  -> Minimal (hasTy gamma rec (subTy (singleSubst n) L))
  -> MinFitsSubst gamma ((natStepArgTy L) ∷ tyNat ∷ gamma) (natStepCompSubCtx n rec gamma)
minNatStepFits {gamma = gamma} {L = L} {n = n} {rec = rec} dn drec =
  minFitsCons firstStep recTyped
  where
  gammaWF : MinCtxWF gamma
  gammaWF = minDerivToCtxWF dn

  base : MinFitsSubst gamma gamma (keepSubstCtx 0 gamma)
  base = minFitsKeepCtx {delta = []} {gamma = gamma} gammaWF

  firstStep : MinFitsSubst gamma (tyNat ∷ gamma) (consSubst n (keepSubstCtx 0 gamma))
  firstStep = minFitsCons base dn

  recTyped : Minimal (hasTy gamma rec (subTy (consSubst n (keepSubstCtx 0 gamma)) (natStepArgTy L)))
  recTyped =
    subst
      (λ T -> Minimal (hasTy gamma rec T))
...
mutual
  ...
  minAssocTmRight (minISucEq d) =
    minISuc (minAssocTmRight d)
  minAssocTmRight (minENatEq dL dnn' _ daa' dStepArg _ dll' _) =
    minConv
      (minENat dL (minAssocTmRight dnn') (minAssocTmRight daa')
        dStepArg (minAssocTmRight dll') dTyRight)
      (minSymTy (minSingleEqSubstTy dL dnn') dTyRight)
    where
    dTyRight = minSingleSubstTy dL (minAssocTmRight dnn')
  minAssocTmRight (minCNatZ _ da _ _ _) =
    da
  minAssocTmRight
    (minCNatS {gamma = gamma} {L = L} {a = a} {l = l} {n = n}
      dL dn da dStepArg dl _) =
    subst
      (λ T -> Minimal (hasTy gamma (subTm (natStepCompSub n (tmElNat a l n)) l) T))
      (natStepTyComp n (tmElNat a l n) gamma L)
      (subst
        (λ u -> Minimal
          (hasTy gamma u
            (subTy (natStepCompSubCtx n (tmElNat a l n) gamma) (natStepTy L))))
        (natStepCompSubCtx-subTm n (tmElNat a l n) gamma l)
        (minSubst dl (minNatStepFits dn (minENat dL dn da dStepArg dl dTyRec))))
    where
    dTyRec = minSingleSubstTy dL dn
  ...
  minAssocTmTy (minISucEq d) = minFNat (minDerivToCtxWF d)
  minAssocTmTy (minENatEq _ _ _ _ _ _ _ dTy) = dTy
  minAssocTmTy (minCNatZ _ _ _ _ dTy) = dTy
  minAssocTmTy (minCNatS _ _ _ _ _ dTy) = dTy
...
mutual
  ...
  minimalToDerivable (minCQtr dL da _ dBranchTy dl _ coh _) =
    cQtr (minimalToDerivable dL) (minimalToDerivable da)
      (minimalToDerivable dBranchTy) (minimalToDerivable dl) (minimalToDerivable coh)
  minimalToDerivable (minFNat wf) =
    fNat (minCtxWFToCtxWF wf)
  minimalToDerivable (minIZero wf) =
    iZero (minCtxWFToCtxWF wf)
  minimalToDerivable (minISuc dn) =
    iSuc (minimalToDerivable dn)
  minimalToDerivable (minISucEq dnn') =
    iSucEq (minimalToDerivable dnn')
  minimalToDerivable (minENat dL dn da _ dl _) =
    eNat (minimalToDerivable dL) (minimalToDerivable dn)
      (minimalToDerivable da) (minimalToDerivable dl)
  minimalToDerivable (minENatEq dL dnn' da daa' _ dl dll' _) =
    eNatEq (minimalToDerivable dL) (minimalToDerivable dnn')
      (minimalToDerivable da) (minimalToDerivable daa')
      (minimalToDerivable dl) (minimalToDerivable dll')
  minimalToDerivable (minCNatZ dL da _ dl _) =
    cNatZ (minimalToDerivable dL) (minimalToDerivable da) (minimalToDerivable dl)
  minimalToDerivable (minCNatS dL dn da _ dl _) =
    cNatS (minimalToDerivable dL) (minimalToDerivable dn)
      (minimalToDerivable da) (minimalToDerivable dl)
  ...
  derivableToMinimal (fNat wf) =
    minFNat (ctxWFToMinCtxWF wf)
  derivableToMinimal (iZero wf) =
    minIZero (ctxWFToMinCtxWF wf)
  derivableToMinimal (iSuc dn) =
    minISuc (derivableToMinimal dn)
  derivableToMinimal (iSucEq dnn') =
    minISucEq (derivableToMinimal dnn')
  derivableToMinimal (eNat dL dn da dl) =
    minENat dLMin dnMin daMin dStepArgMin dlMin dTyMin
    where
    dLMin = derivableToMinimal dL
    dnMin = derivableToMinimal dn
    daMin = derivableToMinimal da
    dlMin = derivableToMinimal dl
    dStepArgMin = minCtxSuffixTy {delta = []} (minDerivToCtxWF dlMin)
    dTyMin = minSingleSubstTy dLMin dnMin
  derivableToMinimal (eNatEq dL dnn' da daa' dl dll') =
    minENatEq dLMin dnn'Min daMin daa'Min dStepArgMin dlMin dll'Min dTyMin
    where
    dLMin = derivableToMinimal dL
    dnn'Min = derivableToMinimal dnn'
    daMin = derivableToMinimal da
    daa'Min = derivableToMinimal daa'
    dlMin = derivableToMinimal dl
    dll'Min = derivableToMinimal dll'
    dStepArgMin = minCtxSuffixTy {delta = []} (minDerivToCtxWF dlMin)
    dTyMin = minSingleSubstTy dLMin (minAssocTmLeft dnn'Min)
  derivableToMinimal (cNatZ dL da dl) =
    minCNatZ dLMin daMin dStepArgMin dlMin dTyMin
    where
    dLMin = derivableToMinimal dL
    daMin = derivableToMinimal da
    dlMin = derivableToMinimal dl
    dStepArgMin = minCtxSuffixTy {delta = []} (minDerivToCtxWF dlMin)
    dTyMin = minSingleSubstTy dLMin (minIZero (minDerivToCtxWF daMin))
  derivableToMinimal (cNatS dL dn da dl) =
    minCNatS dLMin dnMin daMin dStepArgMin dlMin dTyMin
    where
    dLMin = derivableToMinimal dL
    dnMin = derivableToMinimal dn
    daMin = derivableToMinimal da
    dlMin = derivableToMinimal dl
    dStepArgMin = minCtxSuffixTy {delta = []} (minDerivToCtxWF dlMin)
    dTyMin = minSingleSubstTy dLMin (minISuc dnMin)
