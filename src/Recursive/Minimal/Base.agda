...
module Recursive.Minimal.Base where
...
mutual
  ...
  renTmIdBy h tmZero = refl
  renTmIdBy h (tmSuc n) =
    cong tmSuc (renTmIdBy h n)
  renTmIdBy {rho = rho} h (tmElNat a l n) =
    cong₃ tmElNat
      (renTmIdBy h a)
      (renTmIdBy
        (raiseRenIdBy {rho = raiseRen rho} (raiseRenIdBy {rho = rho} h))
...
private
  ...
  subSubNatStepArg-apply : (sigma : Subst) (n : ℕ)
    -> applySubst (compSub (liftSubst sigma) natStepArgSub) n
         ≡ applySubst (compSub natStepArgSub (liftSubst sigma)) n
  subSubNatStepArg-apply sigma zero = refl
  subSubNatStepArg-apply sigma (suc n) =
    applySubst-compSub (liftSubst sigma) natStepArgSub (suc n)
    ∙ liftSubst-apply-suc sigma n
    ∙ renTmKeepSubstBy 1 (applySubst sigma n)
    ∙ sym (subTmRen natStepArgSub sucRen (applySubst sigma n))
    ∙ cong (subTm natStepArgSub) (sym (liftSubst-apply-suc sigma n))
    ∙ sym (applySubst-compSub natStepArgSub (liftSubst sigma) (suc n))

  subSubNatStep-apply : (sigma : Subst) (n : ℕ)
    -> applySubst (compSub (liftSubst (liftSubst sigma)) natStepSub) n
         ≡ applySubst (compSub natStepSub (liftSubst sigma)) n
  subSubNatStep-apply sigma zero = refl
  subSubNatStep-apply sigma (suc n) =
    applySubst-compSub (liftSubst (liftSubst sigma)) natStepSub (suc n)
    ∙ liftSubst-apply-suc (liftSubst sigma) (suc n)
    ∙ cong (renTm sucRen) (liftSubst-apply-suc sigma n)
    ∙ renTmComp sucRen sucRen (applySubst sigma n)
    ∙ renTmKeepSubstBy 2 (applySubst sigma n)
    ∙ sym (subTmRen natStepSub sucRen (applySubst sigma n))
    ∙ cong (subTm natStepSub) (sym (liftSubst-apply-suc sigma n))
    ∙ sym (applySubst-compSub natStepSub (liftSubst sigma) (suc n))
...
subNatStepArgTy : (sigma : Subst) (L : RawType)
  -> subTy (liftSubst sigma) (natStepArgTy L)
       ≡ natStepArgTy (subTy (liftSubst sigma) L)
subNatStepArgTy sigma L =
  subTyComp (liftSubst sigma) natStepArgSub L
  ∙ subTyEq (subSubNatStepArg-apply sigma) L
  ∙ sym (subTyComp natStepArgSub (liftSubst sigma) L)

subNatStepTy : (sigma : Subst) (L : RawType)
  -> subTy (liftSubst (liftSubst sigma)) (natStepTy L)
       ≡ natStepTy (subTy (liftSubst sigma) L)
subNatStepTy sigma L =
  subTyComp (liftSubst (liftSubst sigma)) natStepSub L
  ∙ subTyEq (subSubNatStep-apply sigma) L
  ∙ sym (subTyComp natStepSub (liftSubst sigma) L)
...
renSubNatStepArg-apply : (rho : Ren) (n : ℕ)
  -> applySubst (renSub (raiseRen rho) natStepArgSub) n
       ≡ applySubst (compSubRen natStepArgSub (raiseRen rho)) n
renSubNatStepArg-apply rho zero = refl
renSubNatStepArg-apply rho (suc n) =
  applySubst-renSub (raiseRen rho) natStepArgSub (suc n)
  ∙ cong var (applyRen-raise-suc rho n)
  ∙ sym
      (applySubst-compSubRen natStepArgSub (raiseRen rho) (suc n)
       ∙ cong (applySubst natStepArgSub) (applyRen-raise-suc rho n))

renNatStepArgTy : (rho : Ren) (L : RawType)
  -> renTy (raiseRen rho) (natStepArgTy L)
       ≡ natStepArgTy (renTy (raiseRen rho) L)
renNatStepArgTy rho L =
  renTySub (raiseRen rho) natStepArgSub L
  ∙ subTyEq (renSubNatStepArg-apply rho) L
  ∙ sym (subTyRen natStepArgSub (raiseRen rho) L)

renSubNatStep-apply : (rho : Ren) (n : ℕ)
  -> applySubst (renSub (raiseRen (raiseRen rho)) natStepSub) n
       ≡ applySubst (compSubRen natStepSub (raiseRen rho)) n
renSubNatStep-apply rho zero = refl
renSubNatStep-apply rho (suc n) =
  applySubst-renSub (raiseRen (raiseRen rho)) natStepSub (suc n)
  ∙ cong var
      (applyRen-raise-suc (raiseRen rho) (suc n)
       ∙ cong suc (applyRen-raise-suc rho n))
  ∙ sym
      (applySubst-compSubRen natStepSub (raiseRen rho) (suc n)
       ∙ cong (applySubst natStepSub) (applyRen-raise-suc rho n))

renNatStepTy : (rho : Ren) (L : RawType)
  -> renTy (raiseRen (raiseRen rho)) (natStepTy L)
       ≡ natStepTy (renTy (raiseRen rho) L)
renNatStepTy rho L =
  renTySub (raiseRen (raiseRen rho)) natStepSub L
  ∙ subTyEq (renSubNatStep-apply rho) L
  ∙ sym (subTyRen natStepSub (raiseRen rho) L)
...
mutual
  data Minimal : JForm -> Type where
    ...
    minFNat : {gamma : Ctx}
      -> MinCtxWF gamma
      -> Minimal (isType gamma tyNat)

    minIZero : {gamma : Ctx}
      -> MinCtxWF gamma
      -> Minimal (hasTy gamma tmZero tyNat)

    minISuc : {gamma : Ctx} {n : RawTerm}
      -> Minimal (hasTy gamma n tyNat)
      -> Minimal (hasTy gamma (tmSuc n) tyNat)

    minISucEq : {gamma : Ctx} {n n' : RawTerm}
      -> Minimal (termEq gamma n n' tyNat)
      -> Minimal (termEq gamma (tmSuc n) (tmSuc n') tyNat)

    minENat : {gamma : Ctx} {L : RawType} {a l n : RawTerm}
      -> Minimal (isType (tyNat ∷ gamma) L)
      -> Minimal (hasTy gamma n tyNat)
      -> Minimal (hasTy gamma a (subTy (singleSubst tmZero) L))
      -> Minimal (isType (tyNat ∷ gamma) (natStepArgTy L))
      -> Minimal (hasTy ((natStepArgTy L) ∷ tyNat ∷ gamma) l (natStepTy L))
      -> Minimal (isType gamma (subTy (singleSubst n) L))
      -> Minimal (hasTy gamma (tmElNat a l n) (subTy (singleSubst n) L))

    minENatEq : {gamma : Ctx} {L : RawType} {a a' l l' n n' : RawTerm}
      -> Minimal (isType (tyNat ∷ gamma) L)
      -> Minimal (termEq gamma n n' tyNat)
      -> Minimal (hasTy gamma a (subTy (singleSubst tmZero) L))
      -> Minimal (termEq gamma a a' (subTy (singleSubst tmZero) L))
      -> Minimal (isType (tyNat ∷ gamma) (natStepArgTy L))
      -> Minimal (hasTy ((natStepArgTy L) ∷ tyNat ∷ gamma) l (natStepTy L))
      -> Minimal (termEq ((natStepArgTy L) ∷ tyNat ∷ gamma) l l' (natStepTy L))
      -> Minimal (isType gamma (subTy (singleSubst n) L))
      -> Minimal
           (termEq gamma (tmElNat a l n) (tmElNat a' l' n')
             (subTy (singleSubst n) L))

    minCNatZ : {gamma : Ctx} {L : RawType} {a l : RawTerm}
      -> Minimal (isType (tyNat ∷ gamma) L)
      -> Minimal (hasTy gamma a (subTy (singleSubst tmZero) L))
      -> Minimal (isType (tyNat ∷ gamma) (natStepArgTy L))
      -> Minimal (hasTy ((natStepArgTy L) ∷ tyNat ∷ gamma) l (natStepTy L))
      -> Minimal (isType gamma (subTy (singleSubst tmZero) L))
      -> Minimal (termEq gamma (tmElNat a l tmZero) a (subTy (singleSubst tmZero) L))

    minCNatS : {gamma : Ctx} {L : RawType} {a l n : RawTerm}
      -> Minimal (isType (tyNat ∷ gamma) L)
      -> Minimal (hasTy gamma n tyNat)
      -> Minimal (hasTy gamma a (subTy (singleSubst tmZero) L))
      -> Minimal (isType (tyNat ∷ gamma) (natStepArgTy L))
      -> Minimal (hasTy ((natStepArgTy L) ∷ tyNat ∷ gamma) l (natStepTy L))
      -> Minimal (isType gamma (subTy (singleSubst (tmSuc n)) L))
      -> Minimal
           (termEq gamma (tmElNat a l (tmSuc n))
             (subTm (consSubst (tmElNat a l n) (consSubst n idSubst)) l)
             (subTy (singleSubst (tmSuc n)) L))
...
minDerivToCtxWF (minFNat wf) = wf
minDerivToCtxWF (minIZero wf) = wf
minDerivToCtxWF (minISuc d) = minDerivToCtxWF d
minDerivToCtxWF (minISucEq d) = minDerivToCtxWF d
minDerivToCtxWF (minENat _ d _ _ _ _) = minDerivToCtxWF d
minDerivToCtxWF (minENatEq _ d _ _ _ _ _ _) = minDerivToCtxWF d
minDerivToCtxWF (minCNatZ _ d _ _ _) = minDerivToCtxWF d
minDerivToCtxWF (minCNatS _ d _ _ _ _) = minDerivToCtxWF d
...
mutual
  ...
  minAssocTmLeft (minISucEq d) =
    minISuc (minAssocTmLeft d)
  minAssocTmLeft (minENatEq dL dnn' da _ dStepArg dl _ dTy) =
    minENat dL (minAssocTmLeft dnn') da dStepArg dl dTy
  minAssocTmLeft (minCNatZ dL da dStepArg dl dTy) =
    minENat dL (minIZero (minDerivToCtxWF da)) da dStepArg dl dTy
  minAssocTmLeft (minCNatS dL dn da dStepArg dl dTy) =
    minENat dL (minISuc dn) da dStepArg dl dTy
