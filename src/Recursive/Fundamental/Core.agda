...
module Recursive.Fundamental.Core where
...
open import Recursive.Fundamental.MeasureLemmas public

elNatEqRecFund : {L : RawType} {sigma tau : Subst}
  {aσ aτ l l' x y : RawTerm}
  -> ComputableTmEq (subTy (consSubst tmZero sigma) L) aσ aτ
  -> ({x : RawTerm} -> x =>e tmZero
      -> ComputableTyEq (subTy (consSubst x sigma) L)
           (subTy (consSubst tmZero sigma) L))
  -> ({x k : RawTerm} -> x =>e tmSuc k -> CanonicalNat k
      -> ComputableTyEq (subTy (consSubst x sigma) L)
           (subTy (consSubst (tmSuc k) sigma) L))
  -> ({k k' rec rec' : RawTerm}
      -> CanonicalNatEq k k'
      -> ComputableTmEq (subTy (consSubst k sigma) L) rec rec'
      -> ComputableTmEq (subTy (consSubst (tmSuc k) sigma) L)
           (subTm (consSubst rec (consSubst k sigma)) l)
           (subTm (consSubst rec' (consSubst k' tau)) l'))
  -> CanonicalNatEq x y
  -> ComputableTmEq (subTy (consSubst x sigma) L)
       (tmElNat aσ (subTm (liftSubst (liftSubst sigma)) l) x)
       (tmElNat aτ (subTm (liftSubst (liftSubst tau)) l') y)
elNatEqRecFund {L = L} {sigma = sigma} {aσ = aσ} {aτ = aτ}
  baseEq zeroTy sucTy stepEq (cZeroVEq {t = x} evx evy) =
  let
    baseAtX =
      compTmEq-conv
        {A = subTy (consSubst tmZero sigma) L}
        {B = subTy (consSubst x sigma) L}
        (compTyEq-sym
          {A = subTy (consSubst x sigma) L}
          {B = subTy (consSubst tmZero sigma) L}
          (zeroTy evx))
        baseEq
    caσ , caτ = compTmEq-sides {A = subTy (consSubst x sigma) L} baseAtX
    _ , evaσ , _ = computableResult caσ
    _ , evaτ , _ = computableResult caτ
    baseVal =
      compTmEq-=>e-fwd-right evaτ
        (compTmEq-=>e-fwd-left evaσ baseAtX)
  in
  compTmEq-=>e-back-right (evalElNatZ evy evaτ)
    (compTmEq-=>e-back-left (evalElNatZ evx evaσ) baseVal)
elNatEqRecFund {L = L} {sigma = sigma} {tau = tau}
  {aσ = aσ} {aτ = aτ} {l = l} {l' = l'}
  baseEq zeroTy sucTy stepEq (cSucVEq {t = x} {k = k} {k' = k'} evx evy eqK) =
  let
    ih =
      elNatEqRecFund {L = L} {sigma = sigma} {tau = tau}
        {aσ = aσ} {aτ = aτ} {l = l} {l' = l'}
        baseEq zeroTy sucTy stepEq eqK
    cRecσ , cRecτ = compTmEq-sides {A = subTy (consSubst k sigma) L} ih
    recσ , evRecσ , _ = computableResult cRecσ
    recτ , evRecτ , _ = computableResult cRecτ
    ihVal =
      compTmEq-=>e-fwd-right evRecτ
        (compTmEq-=>e-fwd-left evRecσ ih)
    stepRaw = stepEq eqK ihVal
    stepReady =
      compTmEq-subst
        refl
        (sym (sigmaCompLiftTm k recσ sigma l))
        (sym (sigmaCompLiftTm k' recτ tau l'))
        stepRaw
    stepEl =
      compTmEqElNatSRight evy evRecτ
        (compTmEqElNatSLeft evx evRecσ stepReady)
    ck , _ = canonicalNatEqSides eqK
  in
  compTmEq-conv
    {A = subTy (consSubst (tmSuc k) sigma) L}
    {B = subTy (consSubst x sigma) L}
    (compTyEq-sym
      {A = subTy (consSubst x sigma) L}
      {B = subTy (consSubst (tmSuc k) sigma) L}
      (sucTy evx ck))
    stepEl
...
mutual
  ...
  fundTmEqEnv (iZero wf) _ ee = cZeroVEq evalZero evalZero , tt
  fundTmEqEnv d@(iSuc dn) (acc rec) ee =
    let eqn , _ = fundTmEqEnv dn (rec {y = mDeriv dn} (mDeriv-summand< d ≤-refl)) ee in
    cSucVEq evalSucV evalSucV eqn , tt
  fundTmEqEnv {sigma = sigma} {tau = tau}
    d@(eNat {L = L} {a = a} {l = l} {n = n} dL dn da dl) (acc rec) ee =
    let
      aσ = subTm sigma a
      aτ = subTm tau a
      lσ = subTm (liftSubst (liftSubst sigma)) l
      lτ = subTm (liftSubst (liftSubst tau)) l
      accL =
        rec {y = mDeriv dL}
          (mDeriv-summand< d (≤-sum1-4 (mDeriv dL) (mDeriv dn) (mDeriv da) (mDeriv dl)))
      accn =
        rec {y = mDeriv dn}
          (mDeriv-summand< d (≤-sum2-4 (mDeriv dL) (mDeriv dn) (mDeriv da) (mDeriv dl)))
      acca =
        rec {y = mDeriv da}
          (mDeriv-summand< d (≤-sum3-4 (mDeriv dL) (mDeriv dn) (mDeriv da) (mDeriv dl)))
      accl =
        rec {y = mDeriv dl}
          (mDeriv-summand< d (≤-sum4-4 (mDeriv dL) (mDeriv dn) (mDeriv da) (mDeriv dl)))
      dnEq = proj₁ (fundTmEqEnv dn accn ee)
      aEq =
        compTmEq-subst
          (natZeroTargetTy sigma L)
          refl refl
          (proj₁ (fundTmEqEnv da acca ee))
      tyResult =
        compTyEq-subst
          (sym (subTyComp sigma (singleSubst n) L))
          (sym (subTyComp tau (singleSubst n) L))
          (fundTyEqEnv dL accL (eqEnvCons ee tt dnEq))
      zeroTy : {x : RawTerm}
        -> x =>e tmZero
        -> ComputableTyEq (subTy (consSubst x sigma) L)
             (subTy (consSubst tmZero sigma) L)
      zeroTy =
        λ {x} evx ->
          fundTyEqEnv dL accL
            (eqEnvCons (eqEnvReflLeft ee) tt (cZeroVEq evx evalZero))
      sucTy : {x k : RawTerm}
        -> x =>e tmSuc k
        -> CanonicalNat k
        -> ComputableTyEq (subTy (consSubst x sigma) L)
      ...
      sucTy =
        λ {x} {k} evx ck ->
          fundTyEqEnv dL accL
            (eqEnvCons (eqEnvReflLeft ee) tt (cSucVEq evx evalSucV (canonicalNatRefl ck)))
      stepEq : {k k' r r' : RawTerm}
        -> CanonicalNatEq k k'
        -> ComputableTmEq (subTy (consSubst k sigma) L) r r'
        -> ComputableTmEq (subTy (consSubst (tmSuc k) sigma) L)
             (subTm (consSubst r (consSubst k sigma)) l)
             (subTm (consSubst r' (consSubst k' tau)) l)
      stepEq =
        λ {k} {k'} {r = recVal} {r' = recVal'} eqK recEq ->
          let
            tyK = fundTyEqEnv dL accL (eqEnvCons ee tt eqK)
            tyHead =
              compTyEq-subst
                (sym (natStepArgTargetTy k sigma L))
                (sym (natStepArgTargetTy k' tau L))
                tyK
            recHeadEq =
              compTmEq-subst
                (sym (natStepArgTargetTy k sigma L))
            ...
            stepRaw , _ =
              fundTmEqEnv dl accl
                (eqEnvCons (eqEnvCons ee tt eqK) tyHead recHeadEq)
          in
          compTmEq-subst
            (natStepTargetTy k recVal sigma L)
            refl refl
            stepRaw
      eqEl =
        compTmEq-subst
          ...
          refl refl
          (elNatEqRecFund
            {L = L} {sigma = sigma} {tau = tau}
            {aσ = aσ} {aτ = aτ} {l = l} {l' = l}
            aEq zeroTy sucTy stepEq dnEq)
    in
    eqEl , tyResult
  fundTmEqEnv {sigma = sigma} {tau = tau}
  ...
  fundTmEqEqEnv {sigma = sigma} {tau = tau} d@(iQtrEq {A = A} {a = a} {b = b} da db) (acc rec) ee =
    ...
    computableTyEqQtr-intro {A = subTy sigma A} {C = subTy tau A} tyA
  fundTmEqEqEnv d@(iSucEq dnn') (acc rec) ee =
    let eqn , _ = fundTmEqEqEnv dnn' (rec {y = mDeriv dnn'} (mDeriv-summand< d ≤-refl)) ee in
    cSucVEq evalSucV evalSucV eqn , tt
  fundTmEqEqEnv {sigma = sigma} {tau = tau}
    d@(eNatEq {L = L} {a = a} {a' = a'} {l = l} {l' = l'} {n = n} {n' = n'}
        dL dnn' da daa' dl dll') (acc rec) ee =
    let
      aσ = subTm sigma a
      aτ = subTm tau a'
      lσ = subTm (liftSubst (liftSubst sigma)) l
      ...
      accL =
        rec {y = mDeriv dL}
          (mDeriv-summand< d
            (≤-sum1-6 (mDeriv dL) (mDeriv dnn') (mDeriv da)
              (mDeriv daa') (mDeriv dl) (mDeriv dll')))
      accn =
        rec {y = mDeriv dnn'}
          (mDeriv-summand< d
            (≤-sum2-6 (mDeriv dL) (mDeriv dnn') (mDeriv da)
              (mDeriv daa') (mDeriv dl) (mDeriv dll')))
      accaEq =
        rec {y = mDeriv daa'}
          (mDeriv-summand< d
            (≤-sum4-6 (mDeriv dL) (mDeriv dnn') (mDeriv da)
              (mDeriv daa') (mDeriv dl) (mDeriv dll')))
      acclEq =
        rec {y = mDeriv dll'}
          (mDeriv-summand< d
            (≤-sum6-6 (mDeriv dL) (mDeriv dnn') (mDeriv da)
              (mDeriv daa') (mDeriv dl) (mDeriv dll')))
      dnMain = proj₁ (fundTmEqEqEnv dnn' accn ee)
      dnRight = proj₁ (fundTmEqEqEnv dnn' accn (eqEnvReflRight ee))
      dnSelf =
        compTmEq-trans {A = tyNat}
          {t = subTm sigma n} {u = subTm tau n'} {v = subTm tau n}
          dnMain
          (compTmEq-sym {A = tyNat} dnRight)
      baseEq =
        compTmEq-subst
          (natZeroTargetTy sigma L)
          refl refl
          (proj₁ (fundTmEqEqEnv daa' accaEq ee))
      zeroTy : {x : RawTerm}
        -> x =>e tmZero
        -> ComputableTyEq (subTy (consSubst x sigma) L)
             (subTy (consSubst tmZero sigma) L)
      ...
      stepEq : {k k' r r' : RawTerm}
        ...
        -> ComputableTmEq (subTy (consSubst (tmSuc k) sigma) L)
             ...
             (subTm (consSubst r' (consSubst k' tau)) l')
      stepEq =
        λ {k} {k'} {r = recσ} {r' = recτ} eqK ihVal ->
          let
            tyK =
              fundTyEqEnv dL accL (eqEnvCons ee tt eqK)
            tyHead =
              compTyEq-subst
                (sym (natStepArgTargetTy k sigma L))
                (sym (natStepArgTargetTy k' tau L))
            ...
            recHeadEq =
              compTmEq-subst
                ...
                ihVal
            stepEqRaw , _ =
              fundTmEqEqEnv dll' acclEq
                (eqEnvCons (eqEnvCons ee tt eqK) tyHead recHeadEq)
          in
          compTmEq-subst
            (natStepTargetTy k recσ sigma L)
            refl refl
            stepEqRaw
      tyResult =
        compTyEq-subst
          (sym (subTyComp sigma (singleSubst n) L))
          (sym (subTyComp tau (singleSubst n) L))
          (fundTyEqEnv dL accL (eqEnvCons ee tt dnSelf))
      eqEl =
        compTmEq-subst
          (sym (subTyComp sigma (singleSubst n) L))
          refl refl
          (elNatEqRecFund {L = L} {sigma = sigma} {tau = tau}
            {aσ = aσ} {aτ = aτ} {l = l} {l' = l'}
            baseEq zeroTy sucTy stepEq dnMain)
    in
    eqEl , tyResult
  fundTmEqEqEnv {sigma = sigma} {tau = tau}
    d@(cNatZ {L = L} {a = a} {l = l} dL da dl) (acc rec) ee =
    let
      accL =
        rec {y = mDeriv dL}
          (mDeriv-summand< d (≤-sum1-3 (mDeriv dL) (mDeriv da) (mDeriv dl)))
      acca =
        rec {y = mDeriv da}
          (mDeriv-summand< d (≤-sum2-3 (mDeriv dL) (mDeriv da) (mDeriv dl)))
      aEq = proj₁ (fundTmEqEnv da acca ee)
      tyResult =
        compTyEq-subst
          (sym (subTyComp sigma (singleSubst tmZero) L))
          (sym (subTyComp tau (singleSubst tmZero) L))
          (fundTyEqEnv dL accL (eqEnvCons ee tt (cZeroVEq evalZero evalZero)))
    in
    compTmEqElNatZLeft evalZero aEq , tyResult
  fundTmEqEqEnv {sigma = sigma} {tau = tau}
    d@(cNatS {L = L} {a = a} {l = l} {n = n} dL dn da dl) (acc rec) ee =
    let
      aσ = subTm sigma a
      aτ = subTm tau a
      nσ = subTm sigma n
      ...
      elEq =
        elNatEqRecFund
          ...
          {aσ = aσ} {aτ = aτ} {l = l} {l' = l}
          aEq zeroTy sucTy stepEq dnEq
      cElσ , _ = compTmEq-sides {A = subTy (consSubst nσ sigma) L} elEq
      recσ , evRecσ , _ = computableResult cElσ
      recEq =
        compTmEq-=>e-fwd-left evRecσ elEq
      tyN = fundTyEqEnv dL accL (eqEnvCons ee tt dnEq)
      tyHead =
        compTyEq-subst
          (sym (natStepArgTargetTy nσ sigma L))
          (sym (natStepArgTargetTy nτ tau L))
          tyN
      recHeadEq =
        compTmEq-subst
          (sym (natStepArgTargetTy nσ sigma L))
      ...
      stepRaw , _ =
        fundTmEqEnv dl accl
          (eqEnvCons (eqEnvCons ee tt dnEq) tyHead recHeadEq)
      stepReady =
        compTmEq-subst
          (natStepTargetTy nσ recσ sigma L)
          (sym (sigmaCompLiftTm nσ recσ sigma l))
          (sym (subTmComp tau (natStepCompSub n (tmElNat a l n)) l))
          stepRaw
      tyResult =
        compTyEq-subst
          (sym (subTyComp sigma (singleSubst (tmSuc n)) L))
          (sym (subTyComp tau (singleSubst (tmSuc n)) L))
          (fundTyEqEnv dL accL (eqEnvCons ee tt (cSucVEq evalSucV evalSucV dnEq)))
      eqEl =
        compTmEq-subst
          (sym (subTyComp sigma (singleSubst (tmSuc n)) L))
          refl refl
          (compTmEqElNatSLeft evalSucV evRecσ stepReady)
    in
    eqEl , tyResult
  fundTmEqEqEnv {sigma = sigma} {tau = tau}
