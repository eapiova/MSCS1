...
module Recursive.CompLemmas where
...
evalResultValue evalZero = evalZero
evalResultValue evalSucV = evalSucV
evalResultValue (evalElNatZ evp eva) = evalResultValue eva
evalResultValue (evalElNatS evp evRec evStep) = evalResultValue evStep
...
compTm-=>e-back {A = tyNat} ev (cZeroV evg) =
  cZeroV (eval-=>e-back ev evg)
compTm-=>e-back {A = tyNat} ev (cSucV evg ck) =
  cSucV (eval-=>e-back ev evg) ck
...
compTm-=>e-fwd {A = tyNat} ev (cZeroV evt) =
  cZeroV (eval-=>e-fwd ev evt)
compTm-=>e-fwd {A = tyNat} ev (cSucV evt ck) =
  cSucV (eval-=>e-fwd ev evt) ck

canonicalNatRefl : {t : RawTerm} -> CanonicalNat t -> CanonicalNatEq t t
canonicalNatRefl (cZeroV ev) = cZeroVEq ev ev
canonicalNatRefl (cSucV ev ck) = cSucVEq ev ev (canonicalNatRefl ck)

canonicalNatEqSym : {t u : RawTerm} -> CanonicalNatEq t u -> CanonicalNatEq u t
canonicalNatEqSym (cZeroVEq evt evu) = cZeroVEq evu evt
canonicalNatEqSym (cSucVEq evt evu eqk) = cSucVEq evu evt (canonicalNatEqSym eqk)

canonicalNatEqTrans : {t u v : RawTerm}
  -> CanonicalNatEq t u
  -> CanonicalNatEq u v
  -> CanonicalNatEq t v
canonicalNatEqTrans (cZeroVEq evt evu₁) (cZeroVEq evu₂ evv) =
  cZeroVEq evt evv
canonicalNatEqTrans (cZeroVEq evt evu₁) (cSucVEq evu₂ evv eqk)
  with evalDetTm evu₁ evu₂
... | ()
canonicalNatEqTrans (cSucVEq evt evu₁ eqk₁) (cZeroVEq evu₂ evv)
  with evalDetTm evu₁ evu₂
... | ()
canonicalNatEqTrans (cSucVEq evt evu₁ eqk₁) (cSucVEq evu₂ evv eqk₂)
  with evalDetTm evu₁ evu₂
... | sucEq =
  cSucVEq evt evv
    (canonicalNatEqTrans eqk₁
      (subst (λ x -> CanonicalNatEq x _) (sym (tmSucInj sucEq)) eqk₂))

canonicalNatEqSides : {t u : RawTerm} -> CanonicalNatEq t u -> CanonicalNat t × CanonicalNat u
canonicalNatEqSides (cZeroVEq evt evu) = cZeroV evt , cZeroV evu
canonicalNatEqSides (cSucVEq evt evu eqk) =
  let ck , ck' = canonicalNatEqSides eqk in
  cSucV evt ck , cSucV evu ck'
...
mutual
  ...
  compTm-convAcc A B pA pB eq ct with tyEqAccHead A B pA pB eq
  compTm-convAcc tyTop tyTop pA pB eq ct | shTop = ct
  compTm-convAcc (tySigma A B) (tySigma C D) (acc rsAB) (acc rsCD) (eqAC , eqBD) ct | shSigma =
    let
      a , b , evt , ca , cb = ct
    ...
    compTm-convAcc (subTy (singleSubst a) B) (subTy (singleSubst a) D)
      ...
      eqBaDa cb
  compTm-convAcc (tyEq A a b) (tyEq C c d) (acc rsL) (acc rsR) (eqAC , eqac , eqbd) (evt , eqab) | shEq =
    let
  ...
  compTm-convAcc (tyQtr A) (tyQtr C) (acc rsL) (acc rsR) eqAC (a , evt , ca) | shQtr =
    a , evt ,
    compTm-convAcc A C
      (rsL (tyDepth-base<Qtr A)) (rsR (tyDepth-base<Qtr C))
      eqAC ca
  compTm-convAcc tyNat tyNat pA pB eq ct | shNat = ct
  ...
  compTmEq-reflAcc tyNat pA cty ct = canonicalNatRefl ct
  ...
  compTmEq-symAcc (tyQtr A) (acc rs) (a , b , evt , evu , caa , cbb) =
    b , a , evu , evt , cbb , caa
  compTmEq-symAcc tyNat pA eq = canonicalNatEqSym eq
  ...
  compTmEq-transAcc (tyQtr A) (acc rs) (a , b , evt , evu , caa , cbb) (c , d , evu' , evv , ccc , cdd) =
    a , d , evt , evv , caa , cdd
  compTmEq-transAcc tyNat pA eq₁ eq₂ = canonicalNatEqTrans eq₁ eq₂
  ...
  compTmEq-sidesAcc (tyQtr A) (acc rs) (a , b , evt , evu , caa , cbb) =
    ...
    in
    (a , evt , ca) , (b , evu , cb)
  compTmEq-sidesAcc tyNat pA eq = canonicalNatEqSides eq
  ...
  compTmEq-convAcc A B pA pB eq etu with tyEqAccHead A B pA pB eq
  compTmEq-convAcc tyTop tyTop pA pB eq etu | shTop = etu
  compTmEq-convAcc (tySigma A B) (tySigma C D) (acc rsAB) (acc rsCD) (eqAC , eqBD) etu | shSigma =
    let
    ...
    compTmEq-convAcc (subTy (singleSubst a) B) (subTy (singleSubst a) D)
      pBa pDa tyBaDa eqB ,
    tyDaDc
  compTmEq-convAcc (tyEq A a b) (tyEq C c d) (acc rsL) (acc rsR) (eqAC , eqac , eqbd) (evt , evu , eqab) | shEq =
    let
      pA = rsL (tyDepth-base<Eq A a b)
      pC = rsR (tyDepth-base<Eq C c d)
      eqcb = compTmEq-transAcc A pA (compTmEq-symAcc A pA eqac) eqab
  ...
  compTmEq-convAcc (tyQtr A) (tyQtr C) (acc rsL) (acc rsR) eqAC (a , b , evt , evu , caa , cbb) | shQtr =
    a , b , evt , evu ,
    ...
    compTmEq-convAcc A C
      (rsL (tyDepth-base<Qtr A)) (rsR (tyDepth-base<Qtr C))
      eqAC cbb
  compTmEq-convAcc tyNat tyNat pA pB eq etu | shNat = etu
  ...
  compTyEq-reflAcc tyNat pA ct = tt

  compTyEq-symAcc : (A B : RawType)
      (pA : Acc _<_ (tyDepth A)) (pB : Acc _<_ (tyDepth B))
    -> ComputableTyEqAcc A B pA pB
    -> ComputableTyEqAcc B A pB pA
  compTyEq-symAcc A B pA pB eq with tyEqAccHead A B pA pB eq
  compTyEq-symAcc tyTop tyTop pA pB eq | shTop = tt
  compTyEq-symAcc (tySigma A B) (tySigma C D) (acc rsAB) (acc rsCD) (eqAC , eqBD) | shSigma =
    let
      pA = rsAB (tyDepth-fst<Sigma A B)
      pC = rsCD (tyDepth-fst<Sigma C D)
    ...
    λ c a eqCAca ->
      ...
      compTyEq-symAcc (subTy (singleSubst a) B) (subTy (singleSubst c) D)
        ...
        (eqBD a c eqAac)
  compTyEq-symAcc (tyEq A a b) (tyEq C c d) (acc rsL) (acc rsR) (eqAC , eqac , eqbd) | shEq =
    let
      pA = rsL (tyDepth-base<Eq A a b)
      pC = rsR (tyDepth-base<Eq C c d)
      eqCA = compTyEq-symAcc A C pA pC eqAC
  ...
  compTyEq-symAcc (tyQtr A) (tyQtr C) (acc rsL) (acc rsR) eqAC | shQtr =
    compTyEq-symAcc A C
      (rsL (tyDepth-base<Qtr A)) (rsR (tyDepth-base<Qtr C))
      eqAC
  compTyEq-symAcc tyNat tyNat pA pB eq | shNat = tt
  ...
  compTyEq-transAcc A B C pA pB pC eqAB eqBC
    with tyEqAccHead A B pA pB eqAB | tyEqAccHead B C pB pC eqBC
  compTyEq-transAcc tyTop tyTop tyTop pA pB pC eqAB eqBC | shTop | shTop = tt
  compTyEq-transAcc
    (tySigma A B) (tySigma C D) (tySigma E F)
    (acc rsAB) (acc rsCD) (acc rsEF)
    (eqAC , eqBD) (eqCE , eqDF) | shSigma | shSigma =
    let
      pA = rsAB (tyDepth-fst<Sigma A B)
      pC = rsCD (tyDepth-fst<Sigma C D)
  ...
  compTyEq-transAcc
    ...
    (acc rsAB) (acc rsCD) (acc rsEF)
    (eqAC , eqac , eqbd) (eqCE , eqce , eqdf) | shEq | shEq =
    let
  ...
  compTyEq-transAcc (tyQtr A) (tyQtr C) (tyQtr E) (acc rsA) (acc rsC) (acc rsE) eqAC eqCE | shQtr | shQtr =
    compTyEq-transAcc A C E
      (rsA (tyDepth-base<Qtr A))
      (rsC (tyDepth-base<Qtr C))
      (rsE (tyDepth-base<Qtr E))
      eqAC eqCE
  compTyEq-transAcc tyNat tyNat tyNat pA pB pC eqAB eqBC | shNat | shNat = tt

  compTyEq-sidesAcc : (A B : RawType)
      (pA : Acc _<_ (tyDepth A)) (pB : Acc _<_ (tyDepth B))
    -> ComputableTyEqAcc A B pA pB
    -> ComputableTyAcc A pA × ComputableTyAcc B pB
  compTyEq-sidesAcc A B pA pB eq with tyEqAccHead A B pA pB eq
  compTyEq-sidesAcc tyTop tyTop pA pB eq | shTop = tt , tt
  compTyEq-sidesAcc (tySigma A B) (tySigma C D) (acc rsAB) (acc rsCD) (eqAC , eqBD) | shSigma =
    let
      pA = rsAB (tyDepth-fst<Sigma A B)
      pC = rsCD (tyDepth-fst<Sigma C D)
  ...
  compTyEq-sidesAcc (tyEq A a b) (tyEq C c d) (acc rsL) (acc rsR) (eqAC , eqac , eqbd) | shEq =
    let
      pA = rsL (tyDepth-base<Eq A a b)
      pC = rsR (tyDepth-base<Eq C c d)
      ctyA , ctyC = compTyEq-sidesAcc A C pA pC eqAC
  ...
  compTyEq-sidesAcc (tyQtr A) (tyQtr C) (acc rsL) (acc rsR) eqAC | shQtr =
    compTyEq-sidesAcc A C
      (rsL (tyDepth-base<Qtr A)) (rsR (tyDepth-base<Qtr C))
      eqAC
  compTyEq-sidesAcc tyNat tyNat pA pB eq | shNat = tt , tt
