...
natStepArgTargetTy : (n : RawTerm) (sigma : Subst) (L : RawType)
  -> subTy (consSubst (subTm sigma n) sigma) (natStepArgTy L)
       ≡ subTy sigma (subTy (singleSubst n) L)
natStepArgTargetTy n sigma L =
  subTyComp (consSubst (subTm sigma n) sigma) natStepArgSub L
  ∙ sym (subTyComp sigma (singleSubst n) L)

natStepTargetTy : (n rec : RawTerm) (sigma : Subst) (L : RawType)
  -> subTy (consSubst (subTm sigma rec) (consSubst (subTm sigma n) sigma)) (natStepTy L)
       ≡ subTy sigma (subTy (singleSubst (tmSuc n)) L)
natStepTargetTy n rec sigma L =
  subTyComp (consSubst (subTm sigma rec) (consSubst (subTm sigma n) sigma) ) natStepSub L
  ∙ sym (subTyComp sigma (singleSubst (tmSuc n)) L)
...
mutual
  ...
  minSubstEqTy (minFNat wf) f =
    minReflTy (minFNat (minFitsEqSubstCtxWF f))
  ...
  minSubstEqTm (minIZero wf) f =
    minReflTm (minIZero (minFitsEqSubstCtxWF f))
  minSubstEqTm (minISuc dn) f =
    minISucEq (minSubstEqTm dn f)
  minSubstEqTm (minENat dL dn da dStepArg dl dTy) f =
    asmENatTm dL dn da dStepArg dl dTy f
  ...
  minSubstEqTmRight (minIZero wf) f =
    minSubst (minIZero wf) (minFitsEqLeft f)
  minSubstEqTmRight (minISuc dn) f =
    minISuc (minSubstEqTmRight dn f)
  ...
  minSubstEqTmRight (minEQtr dL dp dA dBranchTy dl dWkA coh dTy) f =
    minConv (minSubst (minEQtr dL dp dA dBranchTy dl dWkA coh dTy) (minFitsEqRight f))
      (minSymTy (minSubstEqTy dTy f) (minSubst dTy (minFitsEqRight f)))
  minSubstEqTmRight (minENat dL dn da dStepArg dl dTy) f =
    minConv (minSubst (minENat dL dn da dStepArg dl dTy) (minFitsEqRight f))
      (minSymTy (minSubstEqTy dTy f) (minSubst dTy (minFitsEqRight f)))
  ...
  minSubstEqTmEq (minISucEq dnn') f =
    minISucEq (minSubstEqTmEq dnn' f)
  minSubstEqTmEq (minENatEq dL dnn' da daa' dStepArg dl dll' dTy) f =
    asmENatEqTmEq dL dnn' da daa' dStepArg dl dll' dTy f
  minSubstEqTmEq (minCNatZ dL da dStepArg dl dTy) f =
    minTransTm Eσ Ea
    where
    Eσ = minSubst (minCNatZ dL da dStepArg dl dTy) (minFitsEqLeft f)
    Ea = minSubstEqTm da f
  minSubstEqTmEq (minCNatS dL dn da dStepArg dl dTy) f =
    asmCNatSTmEq dL dn da dStepArg dl dTy f
  ...
  asmENatTm : {delta target : Ctx} {sigma tau : Subst} {L : RawType} {a l n : RawTerm}
    -> Minimal (isType (tyNat ∷ delta) L)
    -> Minimal (hasTy delta n tyNat)
    -> Minimal (hasTy delta a (subTy (singleSubst tmZero) L))
    -> Minimal (isType (tyNat ∷ delta) (natStepArgTy L))
    -> Minimal (hasTy ((natStepArgTy L) ∷ tyNat ∷ delta) l (natStepTy L))
    -> Minimal (isType delta (subTy (singleSubst n) L))
    -> MinFitsEqSubst target delta sigma tau
    -> Minimal (termEq target
         (subTm sigma (tmElNat a l n)) (subTm tau (tmElNat a l n))
         (subTy sigma (subTy (singleSubst n) L)))
  asmENatTm {target = target} {sigma = sigma} {tau = tau}
    {L = L} {a = a} {l = l} {n = n}
    dL dn da dStepArg dl dTy f =
    subst
      (λ T -> Minimal
        (termEq target
          (tmElNat (subTm sigma a) (subTm (liftSubst (liftSubst sigma)) l) (subTm sigma n))
          (tmElNat (subTm tau a) (subTm (liftSubst (liftSubst tau)) l) (subTm tau n))
          T))
      (sym (subSingleSubstTy sigma n L))
      (minENatEq dL' dnn' da' daa'' dStepArg' dl' dll'' dTy')
    where
    fL = minFitsEqLeft f
    dNat' = minFNat (minFitsEqSubstCtxWF f)
    fitsNat = liftMinFits fL dNat'
    dNatEq = minReflTy dNat'
    fitsNatEq = liftMinFitsEq f dNat' dNatEq
    dL' = minSubst dL fitsNat
    dnn' = minSubstEqTm dn f
    daSub = minSubst da fL
    da' =
      subst
        (λ T -> Minimal (hasTy _ _ T))
        (subSingleSubstTy sigma tmZero L)
        daSub
    daaSub = minSubstEqTm da f
    daa'' =
      subst
        (λ T -> Minimal (termEq _ _ _ T))
        (subSingleSubstTy sigma tmZero L)
        daaSub
    dStepArgSub = minSubst dStepArg fitsNat
    dStepArg' =
      subst
        (λ T -> Minimal (isType (tyNat ∷ target) T))
        (subNatStepArgTy sigma L)
        dStepArgSub
    dStepArgEq = minSubstEqTy dStepArg fitsNatEq
    fitsStep = liftMinFits fitsNat dStepArgSub
    fitsStepEq = liftMinFitsEq fitsNatEq dStepArgSub dStepArgEq
    dlSub = minSubst dl fitsStep
    dlCtx =
      subst
        (λ H -> Minimal
          (hasTy (H ∷ tyNat ∷ target)
            (subTm (liftSubst (liftSubst sigma)) l)
            (subTy (liftSubst (liftSubst sigma)) (natStepTy L))))
        (subNatStepArgTy sigma L)
        dlSub
    dl' =
      subst
        (λ T -> Minimal
          (hasTy (natStepArgTy (subTy (liftSubst sigma) L) ∷ tyNat ∷ target)
            (subTm (liftSubst (liftSubst sigma)) l)
            T))
        (subNatStepTy sigma L)
        dlCtx
    dllSub = minSubstEqTm dl fitsStepEq
    dllCtx =
      subst
        (λ H -> Minimal
          (termEq (H ∷ tyNat ∷ target)
            (subTm (liftSubst (liftSubst sigma)) l)
            (subTm (liftSubst (liftSubst tau)) l)
            (subTy (liftSubst (liftSubst sigma)) (natStepTy L))))
        (subNatStepArgTy sigma L)
        dllSub
    dll'' =
      subst
        (λ T -> Minimal
          (termEq (natStepArgTy (subTy (liftSubst sigma) L) ∷ tyNat ∷ target)
            (subTm (liftSubst (liftSubst sigma)) l)
            (subTm (liftSubst (liftSubst tau)) l)
            T))
        (subNatStepTy sigma L)
    ...
    dTy' =
      subst
        (λ T -> Minimal (isType target T))
        (subSingleSubstTy sigma n L)
        dTySub

  asmENatEqTmEq : {delta target : Ctx} {sigma tau : Subst}
    {L : RawType} {a a' l l' n n' : RawTerm}
    -> Minimal (isType (tyNat ∷ delta) L)
    -> Minimal (termEq delta n n' tyNat)
    -> Minimal (hasTy delta a (subTy (singleSubst tmZero) L))
    -> Minimal (termEq delta a a' (subTy (singleSubst tmZero) L))
    -> Minimal (isType (tyNat ∷ delta) (natStepArgTy L))
    -> Minimal (hasTy ((natStepArgTy L) ∷ tyNat ∷ delta) l (natStepTy L))
    -> Minimal (termEq ((natStepArgTy L) ∷ tyNat ∷ delta) l l' (natStepTy L))
    -> Minimal (isType delta (subTy (singleSubst n) L))
    -> MinFitsEqSubst target delta sigma tau
    -> Minimal (termEq target
         (subTm sigma (tmElNat a l n)) (subTm tau (tmElNat a' l' n'))
         (subTy sigma (subTy (singleSubst n) L)))
  asmENatEqTmEq {target = target} {sigma = sigma} {tau = tau}
    {L = L} {a = a} {a' = a'} {l = l} {l' = l'} {n = n} {n' = n'}
    dL dnn' da daa' dStepArg dl dll' dTy f =
    subst
      (λ T -> Minimal
        (termEq target
          (tmElNat (subTm sigma a) (subTm (liftSubst (liftSubst sigma)) l) (subTm sigma n))
    ...
    dl' =
      subst
        ...
        (subNatStepTy sigma L)
        dlCtx
    dllSub = minSubstEqTmEq dll' fitsStepEq
    dllCtx =
      subst
        (λ H -> Minimal
          (termEq (H ∷ tyNat ∷ target)
  ...
  asmCNatSTmEq : {delta target : Ctx} {sigma tau : Subst}
    {L : RawType} {a l n : RawTerm}
    -> Minimal (isType (tyNat ∷ delta) L)
    -> Minimal (hasTy delta n tyNat)
    -> Minimal (hasTy delta a (subTy (singleSubst tmZero) L))
    -> Minimal (isType (tyNat ∷ delta) (natStepArgTy L))
    -> Minimal (hasTy ((natStepArgTy L) ∷ tyNat ∷ delta) l (natStepTy L))
    -> Minimal (isType delta (subTy (singleSubst (tmSuc n)) L))
    -> MinFitsEqSubst target delta sigma tau
    -> Minimal (termEq target
         (subTm sigma (tmElNat a l (tmSuc n)))
         (subTm tau (subTm (consSubst (tmElNat a l n) (consSubst n idSubst)) l))
         (subTy sigma (subTy (singleSubst (tmSuc n)) L)))
  asmCNatSTmEq {target = target} {sigma = sigma} {tau = tau}
    {L = L} {a = a} {l = l} {n = n}
    dL dn da dStepArg dl dTy f =
    minTransTm Eσ Erc
    where
    rec = tmElNat a l n
    Eσ = minSubst (minCNatS dL dn da dStepArg dl dTy) (minFitsEqLeft f)
    fL = minFitsEqLeft f
    fR = minFitsEqRight f
    dnEq = minSubstEqTm dn f
    dnRight = minSubst dn fR
    dnRightS = minSubstEqTmRight dn f
    fitsN = minFitsEqCons f dnEq dnRight dnRightS
    dNat' = minFNat (minFitsEqSubstCtxWF f)
    fitsNat = liftMinFits fL dNat'
    dNatEq = minReflTy dNat'
    fitsNatEq = liftMinFitsEq f dNat' dNatEq
    ...
    dll'' =
      subst
        ...
        (subNatStepTy sigma L)
        dllCtx
    dTyNRaw = minSubst dL (minFitsCons fL (minSubst dn fL))
    dTyN' =
      subst
        (λ T -> Minimal (isType target T))
        (sym (subTyComp sigma (singleSubst n) L) ∙ subSingleSubstTy sigma n L)
        dTyNRaw
    recEqSingle =
      subst
        (λ T -> Minimal
          (termEq target
            ...
            T))
        (sym (subSingleSubstTy sigma n L))
        (minENatEq dL' dnEq da' daa'' dStepArg' dl' dll'' dTyN')
    recEq =
      subst
        (λ T -> Minimal (termEq target (subTm sigma rec) (subTm tau rec) T))
        (sym (natStepArgTargetTy n sigma L))
        recEqSingle
    dNatR = minFNat (minFitsEqSubstCtxWF f)
    fitsNatR = liftMinFits fR dNatR
    dLR = minSubst dL fitsNatR
    daRSub = minSubst da fR
    daR =
      subst
        (λ T -> Minimal (hasTy _ _ T))
        (subSingleSubstTy tau tmZero L)
        daRSub
    dStepArgRSub = minSubst dStepArg fitsNatR
    dStepArgR =
      subst
        (λ T -> Minimal (isType (tyNat ∷ target) T))
        (subNatStepArgTy tau L)
        dStepArgRSub
    fitsStepR = liftMinFits fitsNatR dStepArgRSub
    dlRSub = minSubst dl fitsStepR
    dlRCtx =
      subst
        (λ H -> Minimal
          (hasTy (H ∷ tyNat ∷ target)
            (subTm (liftSubst (liftSubst tau)) l)
            (subTy (liftSubst (liftSubst tau)) (natStepTy L))))
        (subNatStepArgTy tau L)
        dlRSub
    dlR =
      subst
        (λ T -> Minimal
          (hasTy (natStepArgTy (subTy (liftSubst tau) L) ∷ tyNat ∷ target)
            (subTm (liftSubst (liftSubst tau)) l)
            T))
        (subNatStepTy tau L)
        dlRCtx
    dTyNRRaw = minSubst dL (minFitsCons fR dnRight)
    dTyNR =
      subst
        (λ T -> Minimal (isType target T))
        (sym (subTyComp tau (singleSubst n) L) ∙ subSingleSubstTy tau n L)
        dTyNRRaw
    recRightSingle = minENat dLR dnRight daR dStepArgR dlR dTyNR
    recRight0 =
      subst
        (λ T -> Minimal (hasTy target (subTm tau rec) T))
        (sym (subSingleSubstTy tau n L))
        recRightSingle
    recRight =
      subst
        (λ T -> Minimal (hasTy target (subTm tau rec) T))
        (sym (natStepArgTargetTy n tau L))
        recRight0
    dStepArgTau = minSubst dStepArg (minFitsEqRight fitsN)
    recRightS = minConv recRight (minSymTy (minSubstEqTy dStepArg fitsN) dStepArgTau)
    fitsRec = minFitsEqCons fitsN recEq recRight recRightS
    ErcRaw = minSubstEqTm dl fitsRec
    lhsPath : subTm (consSubst (subTm sigma rec) (consSubst (subTm sigma n) sigma)) l
                ≡ subTm sigma (subTm (consSubst rec (consSubst n idSubst)) l)
    lhsPath = sym (subTmComp sigma (consSubst rec (consSubst n idSubst)) l)
    rhsPath : subTm (consSubst (subTm tau rec) (consSubst (subTm tau n) tau)) l
                ≡ subTm tau (subTm (consSubst rec (consSubst n idSubst)) l)
    rhsPath = sym (subTmComp tau (consSubst rec (consSubst n idSubst)) l)
    typePath : subTy (consSubst (subTm sigma rec) (consSubst (subTm sigma n) sigma)) (natStepTy L)
                 ≡ subTy sigma (subTy (singleSubst (tmSuc n)) L)
    typePath = natStepTargetTy n rec sigma L
    Erc =
      subst
        (λ T -> Minimal
          (termEq target
        ...
        (subst
          ...
          (subst
            (λ u -> Minimal
              (termEq target
                u
                (subTm (consSubst (subTm tau rec) (consSubst (subTm tau n) tau)) l)
                (subTy (consSubst (subTm sigma rec) (consSubst (subTm sigma n) sigma)) (natStepTy L))))
            lhsPath
            ErcRaw))
