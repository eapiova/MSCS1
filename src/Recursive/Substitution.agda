...
module Recursive.Substitution where
...
mutual
  ...
  renTm rho tmZero = tmZero
  renTm rho (tmSuc t) = tmSuc (renTm rho t)
  renTm rho (tmElNat a l n) =
...
mutual
  ...
  renTmEq h tmZero = refl
  renTmEq {rho} {tau} h (tmSuc t) = cong tmSuc (renTmEq {rho = rho} {tau = tau} h t)
  renTmEq {rho} {tau} h (tmElNat a l n) =
    cong₃ tmElNat (renTmEq {rho = rho} {tau = tau} h a)
      (renTmEq {rho = raiseRen (raiseRen rho)} {tau = raiseRen (raiseRen tau)}
        (raiseRen-apply-cong {rho = raiseRen rho} {tau = raiseRen tau}
          (raiseRen-apply-cong {rho = rho} {tau = tau} h)) l)
      (renTmEq {rho = rho} {tau = tau} h n)
...
mutual
  ...
  subTm sigma tmZero = tmZero
  subTm sigma (tmSuc t) = tmSuc (subTm sigma t)
  subTm sigma (tmElNat a l n) =
...
mutual
  ...
  subTmEq h tmZero = refl
  subTmEq {sigma} {tau} h (tmSuc t) = cong tmSuc (subTmEq {sigma = sigma} {tau = tau} h t)
  subTmEq {sigma} {tau} h (tmElNat a l n) =
    cong₃ tmElNat (subTmEq {sigma = sigma} {tau = tau} h a)
      (subTmEq {sigma = liftSubst (liftSubst sigma)} {tau = liftSubst (liftSubst tau)}
        (liftSubst-apply-cong {sigma = liftSubst sigma} {tau = liftSubst tau}
          (liftSubst-apply-cong {sigma = sigma} {tau = tau} h)) l)
      (subTmEq {sigma = sigma} {tau = tau} h n)
...
mutual
  ...
  subTmId tmZero = refl
  subTmId (tmSuc t) = cong tmSuc (subTmId t)
  subTmId (tmElNat a l n) =
    cong₃ tmElNat (subTmId a) (liftId2-subTm l ∙ subTmId l) (subTmId n)
...
mutual
  ...
  subTmRen sigma rho tmZero = refl
  subTmRen sigma rho (tmSuc t) = cong tmSuc (subTmRen sigma rho t)
  subTmRen sigma rho (tmElNat a l n) =
    cong₃ tmElNat (subTmRen sigma rho a)
      (subTmRen (liftSubst (liftSubst sigma)) (raiseRen (raiseRen rho)) l
       ∙ liftComp2-subTm sigma rho l)
      (subTmRen sigma rho n)
...
mutual
  ...
  renTmComp rho tau (tmElQtr l p) =
    cong₂ tmElQtr
      ...
      (renTmComp rho tau p)
  renTmComp rho tau tmZero = refl
  renTmComp rho tau (tmSuc t) = cong tmSuc (renTmComp rho tau t)
  renTmComp rho tau (tmElNat a l n) =
    cong₃ tmElNat (renTmComp rho tau a)
      (renTmComp (raiseRen (raiseRen rho)) (raiseRen (raiseRen tau)) l
       ∙ raiseCompRen2Tm rho tau l)
      (renTmComp rho tau n)
...
mutual
  ...
  renTmSub rho sigma tmZero = refl
  renTmSub rho sigma (tmSuc t) = cong tmSuc (renTmSub rho sigma t)
  renTmSub rho sigma (tmElNat a l n) =
    cong₃ tmElNat (renTmSub rho sigma a)
      (renTmSub (raiseRen (raiseRen rho)) (liftSubst (liftSubst sigma)) l
       ∙ sym (liftRenSub2Tm rho sigma l))
      (renTmSub rho sigma n)
...
abstract
  ...
  mutual
    ...
    subTyComp sigma tau tyNat = refl
    ...
    subTmComp sigma tau tmZero = refl
    subTmComp sigma tau (tmSuc t) = cong tmSuc (subTmComp sigma tau t)
    subTmComp sigma tau (tmElNat a l n) =
      cong₃ tmElNat (subTmComp sigma tau a)
        (subTmComp (liftSubst (liftSubst sigma)) (liftSubst (liftSubst tau)) l
         ∙ liftCompSub2Tm sigma tau l)
        (subTmComp sigma tau n)
