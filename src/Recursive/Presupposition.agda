...
module Recursive.Presupposition where
...
natStepCompSub : RawTerm -> RawTerm -> Subst
natStepCompSub n rec = consSubst rec (consSubst n idSubst)

natStepCompSubCtx : RawTerm -> RawTerm -> Ctx -> Subst
natStepCompSubCtx n rec gamma = consSubst rec (consSubst n (keepSubstCtx 0 gamma))

natStepCompSubCtx-apply : (n rec : RawTerm) (gamma : Ctx) (k : ℕ)
  -> applySubst (natStepCompSubCtx n rec gamma) k ≡ applySubst (natStepCompSub n rec) k
natStepCompSubCtx-apply n rec gamma zero = refl
natStepCompSubCtx-apply n rec gamma (suc zero) = refl
natStepCompSubCtx-apply n rec gamma (suc (suc k)) = keepSubstCtx-apply 0 gamma k

natStepCompSubCtx-subTy : (n rec : RawTerm) (gamma : Ctx) (A : RawType)
  -> subTy (natStepCompSubCtx n rec gamma) A ≡ subTy (natStepCompSub n rec) A
natStepCompSubCtx-subTy n rec gamma = subTyEq (natStepCompSubCtx-apply n rec gamma)

natStepCompSubCtx-subTm : (n rec : RawTerm) (gamma : Ctx) (t : RawTerm)
  -> subTm (natStepCompSubCtx n rec gamma) t ≡ subTm (natStepCompSub n rec) t
natStepCompSubCtx-subTm n rec gamma = subTmEq (natStepCompSubCtx-apply n rec gamma)
...
derivToCtxWF (fNat wf) = wf
derivToCtxWF (iZero wf) = wf
derivToCtxWF (iSuc d) = derivToCtxWF d
derivToCtxWF (iSucEq d) = derivToCtxWF d
derivToCtxWF (eNat _ d _ _) = derivToCtxWF d
derivToCtxWF (eNatEq _ d _ _ _ _) = derivToCtxWF d
derivToCtxWF (cNatZ _ d _) = derivToCtxWF d
derivToCtxWF (cNatS _ d _ _) = derivToCtxWF d
...
natStepArgTyComp : (n : RawTerm) (gamma : Ctx) (L : RawType)
  -> subTy (consSubst n (keepSubstCtx 0 gamma)) (natStepArgTy L)
       ≡ subTy (singleSubst n) L
natStepArgTyComp n gamma L =
  subTyComp (consSubst n (keepSubstCtx 0 gamma)) natStepArgSub L
  ∙ singleSubstCtx-subTy n gamma L

natStepTyComp : (n rec : RawTerm) (gamma : Ctx) (L : RawType)
  -> subTy (natStepCompSubCtx n rec gamma) (natStepTy L)
       ≡ subTy (singleSubst (tmSuc n)) L
natStepTyComp n rec gamma L =
  subTyComp (natStepCompSubCtx n rec gamma) natStepSub L
  ∙ singleSubstCtx-subTy (tmSuc n) gamma L

natStepFitsHelper : {gamma : Ctx} {L : RawType} {n rec : RawTerm}
  -> Derivable (hasTy gamma n tyNat)
  -> Derivable (hasTy gamma rec (subTy (singleSubst n) L))
  -> FitsSubst gamma ((natStepArgTy L) ∷ tyNat ∷ gamma) (natStepCompSubCtx n rec gamma)
natStepFitsHelper {gamma = gamma} {L = L} {n = n} {rec = rec} dn drec =
  fitsCons firstStep recTyped
  where
  gammaWF : CtxWF gamma
  ...
  nTyped : Derivable (hasTy gamma n (subTy (keepSubstCtx 0 gamma) tyNat))
  nTyped = dn

  firstStep : FitsSubst gamma (tyNat ∷ gamma) (consSubst n (keepSubstCtx 0 gamma))
  firstStep = fitsCons base nTyped

  recTyped : Derivable (hasTy gamma rec (subTy (consSubst n (keepSubstCtx 0 gamma)) (natStepArgTy L)))
  recTyped =
    subst (λ T -> Derivable (hasTy gamma rec T))
...
mutual
  ...
  assocTy (iZero wf) = fNat wf
  assocTy (iSuc d) = fNat (derivToCtxWF d)
  assocTy (eNat dL dn _ _) = singleSubstTyHelper dL dn
  ...
  assocTmLeft (iSucEq d) = iSuc (assocTmLeft d)
  assocTmLeft (eNatEq dL dnn' da _ dl _) =
    eNat dL (assocTmLeft dnn') da dl
  assocTmLeft (cNatZ dL da dl) =
    eNat dL (iZero (derivToCtxWF da)) da dl
  assocTmLeft (cNatS dL dn da dl) =
    eNat dL (iSuc dn) da dl
  ...
  assocTmRight (iSucEq d) = iSuc (assocTmRight d)
  assocTmRight (eNatEq dL dnn' _ daa' _ dll') =
    conv
      (eNat dL (assocTmRight dnn') (assocTmRight daa') (assocTmRight dll'))
      (symTy
        (singleEqSubstTyHelper dL dnn')
        (singleSubstTyHelper dL (assocTmRight dnn')))
  assocTmRight (cNatZ dL da dl) = da
  assocTmRight (cNatS {gamma = gamma} {L = L} {a = a} {l = l} {n = n} dL dn da dl) =
    subst
      (λ T -> Derivable (hasTy gamma (subTm (natStepCompSub n (tmElNat a l n)) l) T))
      (natStepTyComp n (tmElNat a l n) gamma L)
      (subst
        (λ u -> Derivable (hasTy gamma u (subTy (natStepCompSubCtx n (tmElNat a l n) gamma) (natStepTy L))))
        (natStepCompSubCtx-subTm n (tmElNat a l n) gamma l)
        (substTmRule dl (natStepFitsHelper dn (eNat dL dn da dl))))
  ...
  assocTmTy (iSucEq d) = fNat (derivToCtxWF d)
  assocTmTy (eNatEq dL dnn' _ _ _ _) = singleSubstTyHelper dL (assocTmLeft dnn')
  assocTmTy (cNatZ dL da _) = singleSubstTyHelper dL (iZero (derivToCtxWF da))
  assocTmTy (cNatS dL dn _ _) = singleSubstTyHelper dL (iSuc dn)
