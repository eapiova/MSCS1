...
module Recursive.Derivability where
...
natStepSub : Subst
natStepSub = replace0By 2 (tmSuc (var (suc zero)))

natStepArgSub : Subst
natStepArgSub = replace0By 1 (var zero)

natStepArgTy : RawType -> RawType
natStepArgTy L = subTy natStepArgSub L

natStepTy : RawType -> RawType
natStepTy L = subTy natStepSub L
...
mutual
  ...
  data Derivable : JForm -> Type where
    ...
    fNat : {gamma : Ctx}
      -> CtxWF gamma
      -> Derivable (isType gamma tyNat)

    iZero : {gamma : Ctx}
      -> CtxWF gamma
      -> Derivable (hasTy gamma tmZero tyNat)

    iSuc : {gamma : Ctx} {n : RawTerm}
      -> Derivable (hasTy gamma n tyNat)
      -> Derivable (hasTy gamma (tmSuc n) tyNat)

    iSucEq : {gamma : Ctx} {n n' : RawTerm}
      -> Derivable (termEq gamma n n' tyNat)
      -> Derivable (termEq gamma (tmSuc n) (tmSuc n') tyNat)

    eNat : {gamma : Ctx} {L : RawType} {a l n : RawTerm}
      -> Derivable (isType (tyNat ∷ gamma) L)
      -> Derivable (hasTy gamma n tyNat)
      -> Derivable (hasTy gamma a (subTy (singleSubst tmZero) L))
      -> Derivable (hasTy ((natStepArgTy L) ∷ tyNat ∷ gamma) l (natStepTy L))
      -> Derivable (hasTy gamma (tmElNat a l n) (subTy (singleSubst n) L))

    eNatEq : {gamma : Ctx} {L : RawType} {a a' l l' n n' : RawTerm}
      -> Derivable (isType (tyNat ∷ gamma) L)
      -> Derivable (termEq gamma n n' tyNat)
      -> Derivable (hasTy gamma a (subTy (singleSubst tmZero) L))
      -> Derivable (termEq gamma a a' (subTy (singleSubst tmZero) L))
      -> Derivable (hasTy ((natStepArgTy L) ∷ tyNat ∷ gamma) l (natStepTy L))
      -> Derivable (termEq ((natStepArgTy L) ∷ tyNat ∷ gamma) l l' (natStepTy L))
      -> Derivable
           (termEq gamma (tmElNat a l n) (tmElNat a' l' n')
             (subTy (singleSubst n) L))

    cNatZ : {gamma : Ctx} {L : RawType} {a l : RawTerm}
      -> Derivable (isType (tyNat ∷ gamma) L)
      -> Derivable (hasTy gamma a (subTy (singleSubst tmZero) L))
      -> Derivable (hasTy ((natStepArgTy L) ∷ tyNat ∷ gamma) l (natStepTy L))
      -> Derivable
           (termEq gamma (tmElNat a l tmZero) a (subTy (singleSubst tmZero) L))

    cNatS : {gamma : Ctx} {L : RawType} {a l n : RawTerm}
      -> Derivable (isType (tyNat ∷ gamma) L)
      -> Derivable (hasTy gamma n tyNat)
      -> Derivable (hasTy gamma a (subTy (singleSubst tmZero) L))
      -> Derivable (hasTy ((natStepArgTy L) ∷ tyNat ∷ gamma) l (natStepTy L))
