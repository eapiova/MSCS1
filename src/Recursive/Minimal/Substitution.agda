...
module Recursive.Minimal.Substitution where
...
mutual
  ...
  minSubst {target = target} {sigma = sigma}
    ...
    dTy' =
      subst
        ...
        dTySub
  minSubst (minFNat wf) fits =
    minFNat (minFitsSubstCtxWF fits)
  minSubst (minIZero wf) fits =
    minIZero (minFitsSubstCtxWF fits)
  minSubst (minISuc dn) fits =
    minISuc (minSubst dn fits)
  minSubst (minISucEq dnn') fits =
    minISucEq (minSubst dnn' fits)
  minSubst {target = target} {sigma = sigma}
    ...
    where
    dn' = minSubst dn fits
    dNat' = minFNat (minFitsSubstCtxWF fits)
    fitsNat = liftMinFits fits dNat'
    dL' = minSubst dL fitsNat
    daSub = minSubst da fits
    da' =
      subst
        (λ T -> Minimal (hasTy _ _ T))
  ...
  minSubst {target = target} {sigma = sigma}
    ...
    subst
      (λ T -> Minimal
        (termEq target
          (tmElNat (subTm sigma a) (subTm (liftSubst (liftSubst sigma)) l) (subTm sigma n))
          (tmElNat (subTm sigma a') (subTm (liftSubst (liftSubst sigma)) l') _)
          T))
      (sym (subSingleSubstTy sigma n L))
      (minENatEq dL' dnn'' da' daa'' dStepArg' dl' dll'' dTy')
    where
    dnn'' = minSubst dnn' fits
    dNat' = minFNat (minFitsSubstCtxWF fits)
    fitsNat = liftMinFits fits dNat'
    dL' = minSubst dL fitsNat
    daSub = minSubst da fits
  ...
  minSubst {target = target} {sigma = sigma}
    ...
    subst
      (λ T -> Minimal
        (termEq target
          (tmElNat (subTm sigma a) (subTm (liftSubst (liftSubst sigma)) l) tmZero)
          (subTm sigma a)
          T))
      (sym (subSingleSubstTy sigma tmZero L))
  ...
  minSubst {target = target} {sigma = sigma}
    ...
    subst
      (λ T -> Minimal
        (termEq target
          (tmElNat (subTm sigma a) (subTm (liftSubst (liftSubst sigma)) l) (tmSuc (subTm sigma n)))
          (subTm sigma (subTm (consSubst (tmElNat a l n) (consSubst n idSubst)) l))
          T))
      (sym (subSingleSubstTy sigma (tmSuc n) L))
      (subst
        (λ u -> Minimal
          (termEq target
            (tmElNat (subTm sigma a) (subTm (liftSubst (liftSubst sigma)) l) (tmSuc (subTm sigma n)))
            u
            (subTy (singleSubst (tmSuc (subTm sigma n))) (subTy (liftSubst sigma) L))))
        (sym (subSigmaCompSubTm sigma n (tmElNat a l n) l))
    ...
    dTy' =
      subst
        (λ T -> Minimal (isType target T))
        (subSingleSubstTy sigma (tmSuc n) L)
        dTySub
  ...
  minSubstTmTy (minEQtr _ _ _ _ _ _ _ dTy) fits =
    minSubst dTy fits
  minSubstTmTy (minIZero wf) fits =
    minFNat (minFitsSubstCtxWF fits)
  minSubstTmTy (minISuc dn) fits =
    minFNat (minFitsSubstCtxWF fits)
  minSubstTmTy (minENat _ _ _ _ _ dTy) fits =
    minSubst dTy fits
