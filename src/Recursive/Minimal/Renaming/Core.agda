...
mutual
  ...
  minRen {target = target} {rho = rho}
    ...
    dTy' =
      subst
        ...
        dTyRen
  minRen (minFNat wf) fits =
    minFNat (renFitsTargetWF fits)
  minRen (minIZero wf) fits =
    minIZero (renFitsTargetWF fits)
  minRen (minISuc dn) fits =
    minISuc (minRen dn fits)
  minRen (minISucEq dnn') fits =
    minISucEq (minRen dnn' fits)
  minRen {target = target} {rho = rho}
    (minENat {L = L} {a = a} {l = l} {n = n}
      dL dn da dStepArg dl dTy) fits =
    subst
      (λ T -> Minimal
        (hasTy target
          (tmElNat (renTm rho a) (renTm (raiseRen (raiseRen rho)) l) (renTm rho n))
          T))
      (sym (renSingleSubstTy rho n L))
      (minENat dL' dn' da' dStepArg' dl' dTy')
    where
    dn' = minRen dn fits
    dNat' = minFNat (renFitsTargetWF fits)
    fitsNat = renFitsKeep fits dNat'
    dL' = minRen dL fitsNat
    daRen = minRen da fits
    da' =
      subst
        (λ T -> Minimal (hasTy _ _ T))
        (renSingleSubstTy rho tmZero L)
        daRen
    dStepArgRen = minRen dStepArg fitsNat
    dStepArg' =
      subst
        (λ T -> Minimal (isType (tyNat ∷ target) T))
        (renNatStepArgTy rho L)
        dStepArgRen
    fitsStep = renFitsKeep fitsNat dStepArgRen
    dlRen = minRen dl fitsStep
    dlCtx =
      subst
        (λ H -> Minimal
          (hasTy (H ∷ tyNat ∷ target)
            (renTm (raiseRen (raiseRen rho)) l)
    ...
    dl' =
      subst
        (λ T -> Minimal
          (hasTy (natStepArgTy (renTy (raiseRen rho) L) ∷ tyNat ∷ target)
            (renTm (raiseRen (raiseRen rho)) l)
            T))
        (renNatStepTy rho L)
        dlCtx
    ...
    dTy' =
      subst
        (λ T -> Minimal (isType target T))
        (renSingleSubstTy rho n L)
        dTyRen
  minRen {target = target} {rho = rho}
    (minENatEq {L = L} {a = a} {a' = a'} {l = l} {l' = l'} {n = n}
      dL dnn' da daa' dStepArg dl dll' dTy) fits =
    subst
      (λ T -> Minimal
        (termEq target
          (tmElNat (renTm rho a) (renTm (raiseRen (raiseRen rho)) l) (renTm rho n))
          (tmElNat (renTm rho a') (renTm (raiseRen (raiseRen rho)) l') _)
          T))
      (sym (renSingleSubstTy rho n L))
    ...
    dll'' =
      subst
        (λ T -> Minimal
          (termEq (natStepArgTy (renTy (raiseRen rho) L) ∷ tyNat ∷ target)
            (renTm (raiseRen (raiseRen rho)) l)
            (renTm (raiseRen (raiseRen rho)) l')
            T))
        (renNatStepTy rho L)
    ...
    dTy' =
      subst
        ...
        dTyRen
  minRen {target = target} {rho = rho}
    (minCNatZ {L = L} {a = a} {l = l} dL da dStepArg dl dTy) fits =
    subst
      (λ T -> Minimal
        (termEq target
          (tmElNat (renTm rho a) (renTm (raiseRen (raiseRen rho)) l) tmZero)
          (renTm rho a)
          T))
      (sym (renSingleSubstTy rho tmZero L))
      (minCNatZ dL' da' dStepArg' dl' dTy')
    where
    dNat' = minFNat (renFitsTargetWF fits)
    fitsNat = renFitsKeep fits dNat'
    dL' = minRen dL fitsNat
    ...
    dTy' =
      subst
        ...
        dTyRen
  minRen {target = target} {rho = rho}
    (minCNatS {L = L} {a = a} {l = l} {n = n} dL dn da dStepArg dl dTy) fits =
    subst
      (λ T -> Minimal
        (termEq target
          (tmElNat (renTm rho a) (renTm (raiseRen (raiseRen rho)) l) (tmSuc (renTm rho n)))
          (renTm rho (subTm (consSubst (tmElNat a l n) (consSubst n idSubst)) l))
          T))
      (sym (renSingleSubstTy rho (tmSuc n) L))
      (subst
        (λ u -> Minimal
          (termEq target
            (tmElNat (renTm rho a) (renTm (raiseRen (raiseRen rho)) l) (tmSuc (renTm rho n)))
            u
            (subTy (singleSubst (tmSuc (renTm rho n))) (renTy (raiseRen rho) L))))
        (sym (renSigmaCompSubTm rho n (tmElNat a l n) l))
        (minCNatS dL' dn' da' dStepArg' dl' dTy'))
    where
    dn' = minRen dn fits
    dNat' = minFNat (renFitsTargetWF fits)
    fitsNat = renFitsKeep fits dNat'
    ...
    dTy' =
      subst
        (λ T -> Minimal (isType target T))
        (renSingleSubstTy rho (tmSuc n) L)
        dTyRen
  ...
  minRenTmTy (minEQtr _ _ _ _ _ _ _ dTy) fits =
    minRen dTy fits
  minRenTmTy (minIZero wf) fits =
    minFNat (renFitsTargetWF fits)
  minRenTmTy (minISuc dn) fits =
    minFNat (renFitsTargetWF fits)
  minRenTmTy (minENat _ _ _ _ _ dTy) fits =
    minRen dTy fits
  ...
  minRenTmEqLeft {target = target} {rho = rho}
    ...
    dTy' =
      subst
        ...
        (renSingleSubstTy rho (tmClass a) L)
        dTyRen
  minRenTmEqLeft (minISucEq dnn') fits =
    minISuc (minRenTmEqLeft dnn' fits)
  minRenTmEqLeft {target = target} {rho = rho}
    (minENatEq {L = L} {a = a} {l = l} {n = n}
      dL dnn' da _ dStepArg dl _ dTy) fits =
    subst
      (λ T -> Minimal
        (hasTy target
          (tmElNat (renTm rho a) (renTm (raiseRen (raiseRen rho)) l) (renTm rho n))
          T))
      (sym (renSingleSubstTy rho n L))
      (minENat dL' dnL' da' dStepArg' dl' dTy')
    where
    dnL' = minRenTmEqLeft dnn' fits
    dNat' = minFNat (renFitsTargetWF fits)
    fitsNat = renFitsKeep fits dNat'
    dL' = minRen dL fitsNat
    daRen = minRen da fits
    da' =
      subst (λ T -> Minimal (hasTy _ _ T)) (renSingleSubstTy rho tmZero L) daRen
    dStepArgRen = minRen dStepArg fitsNat
    dStepArg' =
      subst (λ T -> Minimal (isType (tyNat ∷ target) T))
        (renNatStepArgTy rho L) dStepArgRen
    fitsStep = renFitsKeep fitsNat dStepArgRen
    dlRen = minRen dl fitsStep
    dlCtx =
      subst
    ...
    dTyRen = minRen dTy fits
    dTy' =
      subst (λ T -> Minimal (isType target T)) (renSingleSubstTy rho n L) dTyRen
  minRenTmEqLeft {target = target} {rho = rho}
    (minCNatZ {L = L} {a = a} {l = l} dL da dStepArg dl dTy) fits =
    subst
      (λ T -> Minimal
        (hasTy target
          ...
          T))
      (sym (renSingleSubstTy rho tmZero L))
      (minENat dL' (minIZero (renFitsTargetWF fits)) da' dStepArg' dl' dTy')
    where
    dNat' = minFNat (renFitsTargetWF fits)
    fitsNat = renFitsKeep fits dNat'
    dL' = minRen dL fitsNat
    ...
    dTyRen = minRen dTy fits
    dTy' =
      subst (λ T -> Minimal (isType target T)) (renSingleSubstTy rho tmZero L) dTyRen
  minRenTmEqLeft {target = target} {rho = rho}
    (minCNatS {L = L} {a = a} {l = l} {n = n} dL dn da dStepArg dl dTy) fits =
    subst
      (λ T -> Minimal
    ...
    dTyRen = minRen dTy fits
    dTy' =
      subst (λ T -> Minimal (isType target T)) (renSingleSubstTy rho (tmSuc n) L) dTyRen
  ...
  minRenTmEqTy (minCQtr _ _ _ _ _ _ _ dTy) fits =
    minRen dTy fits
  minRenTmEqTy (minISucEq d) fits =
    minFNat (renFitsTargetWF fits)
  minRenTmEqTy (minENatEq _ _ _ _ _ _ _ dTy) fits =
    minRen dTy fits
  minRenTmEqTy (minCNatZ _ _ _ _ dTy) fits =
    minRen dTy fits
  minRenTmEqTy (minCNatS _ _ _ _ _ dTy) fits =
    minRen dTy fits
